...
<script>
    ...
    function tambahPoin(containerId, inputName) {
        const container = document.getElementById(containerId);

        const row = document.createElement('div');
        row.className = 'poin-row flex items-center gap-2';
    ...
    function hapusPoin(button) {
        const row = button.closest('.poin-row');

        const container = document.getElementById('list-prasyarat');

        if (container.querySelectorAll('.poin-row').length > 1) {
            row.remove();
        }
    }
</script>
